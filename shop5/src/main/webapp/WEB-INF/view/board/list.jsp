...
<tr>
<td>${boardno }</td>
<c:set value="${boardno-1 }" var="boardno"></c:set>
<td style="text-align: left;">
<c:if test="${!empty board.fileurl }"><a href="file/${board.fileurl}" style="text-decoration: none;">@</a></c:if>
...
<td>${board.name }</td>
<td>
<fmt:formatDate value="${board.regdate }" pattern="yyyy-MM-dd" var="rdate"/>
<c:if test="${today == rdate}"><fmt:formatDate value="${board.regdate }" pattern="HH:mm:ss"/></c:if>
<c:if test="${today != rdate }"><fmt:formatDate value="${board.regdate }" pattern="yyyy-MM-dd"/></c:if>
</td>
<td>${board.readcnt }</td>
</tr>
</c:forEach>
